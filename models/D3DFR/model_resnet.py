...
class ResNetAttention(Module):
    """ ResNet backbone
    """
    def __init__(self, input_size, output_dim, block, layers, zero_init_residual=True):
        """ Args:
            input_size: input_size of backbone
            block: block function
            layers: layers in each block
        ...
        assert input_size[0] in [112, 224, 256],\
            "input_size should be [112, 112] or [224, 224]"
        self.inplanes = 64
        # self.pre = Conv2d(215*2, 256*256, kernel_size=1)
        self.conv1 = Conv2d(3, 64,
                            kernel_size=7,
                            stride=2,
                            padding=3,
        ...
        self.conv1_1 = Conv2d(64, 64, kernel_size=1, stride=1, padding=0)
        self.bn1_1 = BatchNorm2d(64)
        self.relu1_1 = ReLU(inplace=True)
        ...
        # self.dropout = Dropout()
        # if input_size[0] == 256:
        #     self.fc = Linear(2048 * 8 * 8, 232)
        # else:
        #     self.fc = Linear(2048 * 7 * 7, 512)
        # self.bn_o2 = BatchNorm1d(512)
        ##3dmm coeffs 200+26+3+2+1(id+exp+rotation+trans+scale) and pupil position 2+2
        self.fc = Linear(2048, output_dim)
    ...
    def forward(self, x, mask):
        ...
        # y = self.pre(y)
        # # print(y.shape)
        # y = y.view(batch,1,256,256)
        # # print(y.shape)
        # # print(x.shape)
        # x = torch.cat((x,y), dim=1)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x) #128x128
        # modify
        # y = self.conv2(x)
        # y = self.bn2(y)
        # y = self.relu2(y)
...
# class ResNetDropout(Module):
class ResNetDropout(Module):
    """ ResNet backbone
    """
    def __init__(self, input_size, output_dim, init_path=None):
        """ Args:
            input_size: input_size of backbone
            block: block function
            layers: layers in each block
        ...
        assert input_size[0] in [112, 224, 256], \
            "input_size should be [112, 112] or [224, 224]"
        backbone = ResNet_50_RG(input_size=input_size, output_dim=output_dim)
        if init_path and os.path.isfile(init_path):
            state_dict = filter_state_dict(torch.load(init_path, map_location='cpu'))
            backbone.load_state_dict(state_dict)
            print("Loading init recon %s from %s" % ('resnet without dropout', init_path))
        self.backbone = backbone
        self.fianl_layers = nn.ModuleList([Dropout()])
        for m in self.fianl_layers:
            nn.init.constant_(m.weight, 0.)
            nn.init.constant_(m.bias, 0.)
...
def ResNet_50_Dropout(input_size, output_dim, init_path, **kwargs):
    model = ResNetDropout(input_size, output_dim, init_path, **kwargs)
    return model
...
def ResNet_Attention(input_size, output_dim, **kwargs):
    model = ResNetAttention(input_size, output_dim, Bottleneck, [3, 4, 6, 3], **kwargs)
    return model
