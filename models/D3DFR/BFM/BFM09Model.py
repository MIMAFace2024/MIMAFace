...
class BFM09ReconModel(BaseReconModel):
    ...
    def forward(self, coeffs, render=True):
        ...
        if render:
            ...
            return {'rendered_img': rendered_img, #[B H W 4] , range[0,255]
                    'rendered_img_gray': rendered_img_gray, #[B H W 4], range[0,255]
                    'lms_proj': lms_proj, #[B 68 2],float
                    'face_texture': face_texture, # [B, 35709, 3]
                    'vs': vs_t,  # [B, 35709, 3], vertex position after rigid transform
                    'tri': self.tri, # [70789, 3], trimap
                    'color': face_color,
                    'gray_color': gray_color} # [B, 35709, 3] range[0,255]
        else:
